...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * used like any others, but additionally support faster lookup    // treeNodes在数量很多时查找更快
     * when overpopulated. However, since the vast majority of bins in
     * normal use are not overpopulated, checking for existence of
     * tree bins may be delayed in the course of table methods. // 可能在判断是否存在treebin时延迟
     *
     * Tree bins (i.e., bins whose elements are all TreeNodes) are
     * ordered primarily by hashCode, but in the case of ties, if two // 默认hashCode排序，是Comparable时，调用compareTo方法排序
     * elements are of the same "class C implements Comparable<C>",
     * type then their compareTo method is used for ordering. (We
     * conservatively check generic types via reflection to validate
     * this -- see method comparableClassFor).  The added complexity // comparableClassFor方法??
     * of tree bins is worthwhile in providing worst-case O(log n)
     * operations when keys either have distinct hashes or are       // 保证最坏情况下O(log n)
     ...
     * little difference.)
     *
     * Because TreeNodes are about twice the size of regular nodes, we // 树节点是正常节点两倍，因此在结构足够多时才使用
     * use them only when bins contain enough nodes to warrant use
     * (see TREEIFY_THRESHOLD). And when they become too small (due to
     * removal or resizing) they are converted back to plain bins.  In // 当节点太少时，会从树节点退化为普通节点
     * usages with well-distributed user hashCodes, tree bins are
     * rarely used.  Ideally, under random hashCodes, the frequency of  // 这一段啥意思，泊松分布是啥？？
    ...
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) { // 如果达到最大值时，不会进行任何扩容操作，直接返回了
                ...
                newThr = oldThr << 1; // double threshold  // 同时threshold也增大为之前的两倍
        }
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null)
                        ...
                        if (loTail != null) {
                            loTail.next = null;
                            newTab[j] = loHead; // TODO ??
                        }
                        if (hiTail != null) {
                            hiTail.next = null;
                            newTab[j + oldCap] = hiHead; // TODO ??
                        }
                    }
                }
            }
